  ...
  return (
    <>
      {data?.order_details?.map((detail: any) => (
        <React.Fragment key={detail?.item_name}>
              ...
              {isDealer ? (
                ...
                <>
                  <div className="col-lg-4 col-md-2 col-12 text-center text-md-end ">
                    ...
                    {data?.order_status === 'Order Delivered' && !data?.request_for_replacement && query?.status !== 'replaced-orders' && (
                      <button
          ...
          <ReplacementModal
            showReplacementModal={modalOpen}
            handleCloseReplacementModal={() => setModalOpen(false)}
            orderId={data.name || ''}
